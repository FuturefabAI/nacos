...
public class TenantCapacityMapperByOracle extends AbstractMapper implements TenantCapacityMapper {
    ...
    public MapperResult getCapacityList4CorrectUsage(MapperContext context) {
        String id = (String) context.getWhereParameter(FieldConstant.ID);
        int limitSize = (Integer) context.getWhereParameter(FieldConstant.LIMIT_SIZE);

        String sql = "SELECT * FROM ("
                + "  SELECT id, tenant_id, ROWNUM rnum FROM tenant_capacity WHERE id > ?"
                + "  ORDER BY id"
                + ") WHERE rnum <= ?";

        return new MapperResult(sql, CollectionUtils.list(id, limitSize));
    }
}